...
class RentalsRepository extends EntityRepository {
    ...
    public function findAll(): array {
        ...
        $requete->bindParam(':movie_id', $movie_id, PDO::PARAM_INT);
        $requete->bindParam(':movie_id', $movie_id, PDO::PARAM_INT);
        $requete->execute();
    ...
    public function MoviesRentals(): array {
        $requete = $this->cnx->prepare("
            SELECT 
                m.movie_title,
            ...
            JOIN 
                Movies m ON r.movie_id = m.id
            WHERE 
                r.rental_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 6 MONTH) AND CURDATE()
            GROUP BY 
                m.movie_title, DATE_FORMAT(r.rental_date, '%Y-%m')
            ORDER BY 
                DATE_FORMAT(r.rental_date, '%Y-%m') ASC
        ");
        $requete->execute();
        $answer = $requete->fetchAll(PDO::FETCH_OBJ);
